...
def detect_lang(text):
    """
    Detects language of a text using langdetect (offline and stable).
    """
    # Tomamos una muestra del texto para que sea rápido (primeros 1000 caracteres)
    chunk = text[:1000] if len(text) > 1000 else text
    
    try:
        lang = detect(chunk)
    except Exception:
        # Si falla (ej. texto vacío o solo números), asumimos inglés por defecto
        return 'en'
        
    # langdetect devuelve 'zh-cn' o 'zh-tw', bertalign suele esperar 'zh'
