...
def align_segments(segments_ch, segments_en):
    ...
    vecs_ch2 = vecs_ch3 = vecs_ch4 = vecs_ch5 = None
    vecs_en2 = vecs_en3 = vecs_en4 = vecs_en5 = None
    ...
    if M > 4: 
        combined_ch_segments_5 = [
            segments_ch[i] + " " + segments_ch[i+1] + " " + segments_ch[i+2] + " " + segments_ch[i+3] + " " + segments_ch[i+4]
            for i in range(M-4)
        ]
        vecs_ch5 = model.encode(combined_ch_segments_5, normalize_embeddings=True)
    ...
    sim_1to5 = np.dot(vecs_ch, vecs_en5.T) if vecs_en5 is not None else None 
    sim_5to1 = np.dot(vecs_ch5, vecs_en.T) if vecs_ch5 is not None else None 
    ...
    for i in range(M+1):
        for j in range(N+1):
            ...
            if i > 0 and j > 4 and sim_1to5 is not None:
                score = DP[i-1][j-5] + sim_1to5[i-1, j-5]
                if score > DP[i, j]:
                    DP[i, j] = score
                    backpointer[i][j] = ("1-5", i-1, j-5)
            # 5-1 (NUEVO)
            if i > 4 and j > 0 and sim_5to1 is not None:
                score = DP[i-5][j-1] + sim_5to1[i-5, j-1]
                if score > DP[i, j]:
                    DP[i, j] = score
                    backpointer[i][j] = ("5-1", i-5, j-1)
    ...
    while i > 0 or j > 0:
        ...
        if action == "1-1":
            ...
            combined = ' '.join(segments_en[pj:pj+5])
            aligned.append((segments_ch[pi], combined))
            i, j = pi, pj 
        elif action == "5-1": 
            combined = ' '.join(segments_ch[pi:pi+5])
            aligned.append((combined, segments_en[pj]))
            i, j = pi, pj 
...
def calculate_and_print_metrics(global_stats, total_elapsed):
    ...
    total_aligned_pairs = (global_stats["1-1"] + global_stats["1-2"] + 
                        ...
                        + global_stats["1-5"] + global_stats["5-1"])
    ...
    ch_used_in_pairs = (global_stats["1-1"] * 1 + global_stats["1-2"] * 1 + 
                        ...
                        global_stats["4-1"] * 4 + global_stats["1-5"] * 1 + global_stats["5-1"] * 5)                             
    ...
    en_used_in_pairs = (global_stats["1-1"] * 1 + global_stats["1-2"] * 2 + 
                        ...
                        global_stats["4-1"] * 1 + global_stats["1-5"] * 5 + global_stats["5-1"] * 1)
    ...
    print(f"Alineamientos 1-5 (Ch: 1, En: 5): {global_stats['1-5']}") # <-- NUEVO
    print(f"Alineamientos 5-1 (Ch: 5, En: 1): {global_stats['5-1']}")
...
if __name__ == '__main__':
    ...
    ch_suffix = "ch.txt"
    en_suffix = "en.txt"
    output_file = "final_awe_stats.txt"
    ...
    resultados = process_all_files("chapter\\.")
