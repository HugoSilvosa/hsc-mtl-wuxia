...
class Config:
    ...
    # output_dir: Path = base_dir / "models" / "gemma3"
    ...
    translate_file: str = "hunyuan_mt.txt"
    results_file: str = "llm_results.txt"
    
    model_ckpt: str = "tencent/Hunyuan-MT-7B"
...
def load_model_resources(config: Config):
    """Loads tokenizer and model for Hunyuan-MT."""
    print(f"Loading tokenizer: {config.model_ckpt}...")
    
    # Hunyuan requiere trust_remote_code=True
    tokenizer = AutoTokenizer.from_pretrained(config.model_ckpt, trust_remote_code=True)
    tokenizer.padding_side = "left" # Correcto para decoder-only

    # Si no hay pad_token definido, usar eos_token (común en Llama/Hunyuan)
...
def translate_batch_hunyuan(
    ...
    """
    Función vectorizada adaptada para Hunyuan-MT (Fix: token_type_ids).
    """
    prompts_as_strings = []

    # 1. Preparar prompts individuales
    ...
    # === FIX CRÍTICO ===
    # Eliminamos token_type_ids si existe, ya que Hunyuan no lo soporta
    ...
    for t in output_texts:
        res = t.replace("English Output:", "").strip()
        cleaned_texts.append(res)
        ...
        for prompt_idx in PROMPT_IDS:
            ...
            for mode_name, n_shots in SHOTS_CONFIG.items():
                ...
                for i in tqdm(batch_starts, desc=progress_desc):
                    ...
                    try:
                        # 2. Inferencia en Batch (Usando la función adaptada)
                        translations = translate_batch_hunyuan(
